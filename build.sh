...
function verifyCurrentBackup {
	...
	[[ $actualNumberOfWords -gt $expectedNumberOfWords ]] && echo "${BASH_SOURCE[0]}: unexpected number of records, verify and update expectedNumberOfWords: expected number of words: $expectedNumberOfWords , found: $actualNumberOfWords" && exit 1
